...
A condição de fase inicial entre as espaçonaves, conforme a Tabela~\ref{tab:cap4_2_longitude}, foi imposta ao perseguidor no instante inicial da simulação.
...
A Figura~\ref{fig:cap4_1Posições_perseguidor_alvo} mostra as posições iniciais do perseguidor e do alvo, bem como os instantes de aplicação de $\Delta v_1$ e $\Delta v_2$. A aplicação de $\Delta v_2$ marca o encerramento desta fase da missão de \gls{rvdb}.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{4 Resultados e discussoes/Imagens/1 App longa distancia/1 Transferência de órbita.png}
\caption{Posições inicial e final das espaçonaves durante a primeira fase da missão de \gls{rvdb}.}
    \label{fig:cap4_1Posições_perseguidor_alvo}
\end{figure}

A seguir, na Figura~\ref{fig:cap4_2Longitudes_perseguidor_alvo}, a diferença de longitude entre as espaçonaves ao longo do tempo é apresentada.
...
Ao final da aproximação de longa distância, no instante $6.596$ s, a longitude do alvo é $49,3801^{\circ}$ e a do perseguidor é $49,3760^{\circ}$, resultando em diferença de $0,0041^{\circ}$ ($7,15\times 10^{-5}$ rad), correspondente a $506,06$ m de separação relativa.

A Tabela~\ref{tab:cap4_3_longitude_instantes} resume longitudes em instantes selecionados e a respectiva diferença de fase.
O perseguidor levou $6.596$ s (01h49min) para atingir a condição de término desta fase e cerca de $6.598$ segundos ($01$h$49$min)  para se aproximar suficientemente do alvo para finalizar a primeira fase da missão de \gls{rvdb}.
...
$6.598$  & $49,3760^{\circ}$   & $49,3801^{\circ}$   & $-0,0041^{\circ}$ \\
\hline
\end{tabularx}
...
\caption{Evolução do raio orbital do perseguidor ao longo do tempo.}
...
A Tabela~\ref{tab:cap4_comp_alt_tempos} apresenta novos valores de altitude e tempos obtidos após a mudança nos parâmetros iniciais do perseguidor.
...
\hline
Tempo total (após drift)             & segundo& 7.498  & 34.873 & 27.375  & 365\% \\
\hline
\end{tabularx}
\label{tab:cap4_comp_alt_tempos}
\end{table}

O tempo total da aproximação de longa distância (desde a injeção do perseguidor em sua órbita inicial até o instante final do \emph{drift} orbital) do cenário (A) foi de $7.498$ segundos, enquanto que no cenário (B) foi de $34.873$ segundos, ou seja, $365\%$ a mais do que o cenário inicial.

\subsubsection{Consumo de Delta-V}
Outro parâmetro analisado foi o consumo de $\Delta v$.
Os parâmetros obtidos estão na Tabela~\ref{tab:cap4_comp_deltav} e foram calculados através da equação \emph{vis-viva}.
