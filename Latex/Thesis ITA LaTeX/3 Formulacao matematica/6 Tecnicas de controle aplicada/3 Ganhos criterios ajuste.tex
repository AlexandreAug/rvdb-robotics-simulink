...
Dessa forma, os ganhos $K_{p,B}$, $K_{d,B}$, $K_{p,\theta}$ e $K_{d,\theta}$ foram ajustados iterativamente por meio de simulações no ambiente descrito no Capítulo~\ref{sec:Estrategias_RVDB}, seguindo um processo de tentativa e erro, sempre buscando a estabilidade, ou seja, convergência da simulação.

Este ajuste iterativo foi conduzido com base em critérios de sucesso e desempenho pré-definidos, como visto na Seção~\ref{sec:criterios_desempenho}, acrescidos de:
...
Em síntese, embora técnicas formais de sintonia estejam disponíveis na literatura, a calibração dos ganhos neste trabalho foi realizada de forma empírica, porém guiada por critérios de desempenho pré-estabelecidos.