...
onde $\{\vec{a}_1,\vec{a}_2,\vec{a}_3\}$ e $\{\vec{b}_1,\vec{b}_2,\vec{b}_3\}$ são bases ortonormais de $N$ e $B$, respectivamente. Cada elemento $C_{ij}^{B/N}$ é o cosseno do ângulo entre o $i$-ésimo versor de $B$ e o
$j$-ésimo versor de $N$

\begin{equation}
C_{ij}^{B/N} = \vec{b}_i \cdot \vec{a}_j.
\end{equation}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
A matriz $\mathbf{C}^{B/N}$ é a \gls{dcm} que realiza a mudança de base de componentes de um vetor do referencial $N$ para o referencial $B$, assumindo que $\{\vec{a}_1,\vec{a}_2,\vec{a}_3\}$ e $\{\vec{b}_1,\vec{b}_2,\vec{b}_3\}$ são bases ortonormais em $\mathbb{R}^3$.
Por construção, suas linhas e colunas são mutuamente ortogonais e possuem norma unitária, de modo que $\mathbf{C}^{B/N}$ é uma matriz ortogonal (ortonormal).
...
Em muitas referências, o termo "matriz ortogonal" já pressupõe a condição de ortonormalidade e $\det=+1$.
...
pois, sendo $\mathbf{C}^{B/N}$ ortogonal, sua inversa coincide com a transposta, o que corresponde à rotação inversa (transformação de $B$ para $N$).
...
Partindo de \eqref{eq:relacao_inversa_dcm}, e tomando a derivada temporal \emph{vista a partir de} $N$ (isto é, $\frac{d}{dt}\big|_A$), obtém-se
...
onde $\frac{d}{dt}\big|_A\,\vec{a}_i=\vec{0}$, pois os versores do referencial $N$ são fixos quando a derivada é tomada em $N$.
