...
Nesta pesquisa, a definição dos referenciais do manipulador de três juntas revolutas segue a convenção clássica de \gls{dh} para representar a transformação geométrica entre eles \cite{craig2005}.

\subsection{Cinemática direta, convenção e definição dos eixos do manipulador robótico}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{2 Formulacao matematica/2.4 Manipulador robotico/Imagens_MR/Craig_3_5.png}
...
O sistema de coordenadas inercial $\{I\}$ é utilizado como referência absoluta, enquanto o referencial da base $\{B\}$ tem sua origem no centro de massa da base física e seus eixos estão alinhados conforme a orientação do corpo.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{2 Formulacao matematica/2.4 Manipulador robotico/Imagens_MR/Lateral direita com eixos.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{2 Formulacao matematica/2.4 Manipulador robotico/Imagens_MR/Craig_3_6_RotacaoJuntas.png}
...
Os parâmetros $a_i$ permanecem nulos porque todos os eixos de junta se interceptam no mesmo ponto no modelo orientacional, e os termos $d_i$ são definidos como zero por não haver deslocamentos ao longo de $\hat{Z}_i$ nesta etapa. As variáveis articulares $\theta_i$ correspondem diretamente às rotações das três juntas em torno de seus respectivos eixos $\hat{Z}_i$.
...
Multiplicando essas quatro operações na ordem prescrita (direita para a esquerda), obtém-se a matriz homogênea completa \cite{craig2005}
