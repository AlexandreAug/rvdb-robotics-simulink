...
\subsection{Movimento orbital absoluto}
\label{sec:movimento_orbital_absoluto}
...
O \emph{target} (sem força de controle) obedece:
\begin{equation}
\label{eq:movimentoTarget}
\ddot{\vec r_t} = \frac{-\mu}{\vec {r^3}}\vec {r_t}.
\end{equation}
...
\ddot{\vec r_c}
=
-\frac{\mu}{\vec {r_c^3}}\vec r_c
+
\frac{\vec F}{m_c},
\end{equation}

onde $\vect F$ é um tipo de força não-gravitacional, como uma força de controle, aplicada no \emph{chaser} para se aproximar do \emph{target}.
A Figura~\eqref{fig:movimento_ref_absoluto} apresenta o modelo físico da espaçonave do tipo manipulador robótico no contexto do movimento descrito no referencial inercial geocêntrico.
Nesse sistema, as posições do alvo e da espaçonave visitante são expressas em relação ao centro da Terra, possibilitando a análise do movimento absoluto.

\begin{figure}[ht]
\centering
\includegraphics[width=0.9\linewidth]{2 Formulacao do problema/Imagens/Posicao absoluta.png}
