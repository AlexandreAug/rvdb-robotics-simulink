...
A estratégia de controle empregada é realizada através do uso de controladores do tipo proporcional$-$derivativos (PD).
...
Cada sistema de referencia é empregado conforme a necessidade de modelagem:
o referencial inercial descreve a dinâmica absoluta da espaçonave;
...
\section{Hipóteses de simulação e critérios de validação}
...
O \emph{chaser} possuirá desvios em sua posição e em suas velocidades iniciais, enquanto que o \emph{target} será tratado como referência, servindo como orientação fixa no referencial local LVLH.
...
(ii) o controle absoluto de atitude no referencial inercial;
