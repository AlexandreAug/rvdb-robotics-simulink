...
\begin{table}[ht]
\centering
\caption{Parâmetros de Denavit--Hartenberg (DH clássico)}
...
a partir de uma posição e orientação desejada do efetuador
${}^0 \mathbf{T}_E$.
Embora possua complexidade adicional, a cinemática inversa permite que o manipulador seja comandado em termos da pose da ferramenta, o que ajuda no planejamento de trajetórias seguras de aproximação e captura feita pela espaçonave \emph{target}. 

Enquanto que a cinemática direta possui solução única e direta, a cinemática inversa pode admitir múltiplas soluções, ou até mesmo nenhuma solução, pois é dependente da posição desejada em relação ao espaço de trabalho alcançável. 
% Assim, o problema de cinemática inversa pode ser classificado como:
% \begin{enumerate}[label=\roman*)]
%     \item \textbf{Determinado}:
%     quando existe uma solução única para o conjunto de variáveis articulares;
%     \item \textbf{Redundante}:
%     quando há mais de uma configuração articular capaz de atingir a mesma pose do efetuador;
%     \item \textbf{Indeterminado}:
%     quando a pose desejada não está contida no espaço de trabalho do manipulador.
% \end{enumerate}

% Certos métodos podem ser empregados na resolução do problema da cinemática inversa, entre eles:
% \begin{enumerate}[label=\roman*)]
%     \item \textbf{Métodos analíticos}:
%     buscam expressões fechadas para os ângulos das juntas;
%     \item \textbf{Métodos numéricos}:
%     utilizam iterações sucessivas baseadas em otimização, permitindo lidar com manipuladores de maior complexidade, porém ao custo de maior esforço computacional;
%     \item \textbf{Métodos híbridos}:
%     combinam as abordagens anteriores, resolvendo e refinando os resultados por técnicas numéricas.
% \end{enumerate}

Neste trabalho, a cinemática inversa não foi empregada, pois as trajetórias foram especificadas diretamente no espaço articular,
e a posição da porta de docking era previamente conhecida ou especificada no referencial da espaçonave visitante, o que dispensou o uso da cinemática inversa. 
% --------------------------------------------------------
% Comentário:
