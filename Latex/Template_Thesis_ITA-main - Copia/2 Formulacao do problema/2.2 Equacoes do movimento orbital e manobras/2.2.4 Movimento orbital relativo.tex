...
O movimento orbital relativo é formulado a partir da diferença entre as posições da espaçonave visitante e do alvo previamente descritos no ECI, conforme apresentado na Figura~\eqref{fig:movimento_relativo_LVLH}.

\begin{figure}[ht]
\centering
\includegraphics[width=0.9\linewidth]{2 Formulacao do problema/Imagens/Posicao relativa LVLH.png}
\caption{Movimento relativo das espaçonaves em relação ao referencial LVLH.}
\label{fig:movimento_relativo_LVLH}
\end{figure}

Sendo assim, o vetor relativo $s$ expressa a posição do \emph{chaser} em relação ao \emph{target}, e as equações diferenciais permitem descrever a dinâmica da aproximação entre as duas espaçonaves \cite{fehse2003}.
...
\ddot{\vec s} = \left(-\mu \frac{\vect r_c}{\vect {r_c}^3} + \frac{\vect F}{m_c}\right) - \left(-\mu \frac{\vect r_t}{\vect {r_t}^3}\right)
\end{equation}
...
Para órbitas circulares, esta formulação se reduz às equações linearizadas conhecidas como Equações de Hill ou Hill/Clohessy--Wiltshire (HCW) que serão apresentadas a seguir.
