...
O termo $\vec F_p$ representa a força não gravitacional aplicada ao perseguidor, associada ao comando de controle translacional.
...
-
\left(-\mu_\oplus \frac{\vec r_p}{r_p^3} + \frac{\vec F_p}{m_p}\right).
\end{equation}
...
\ddot{\vec s}
=
\vec f_g(\vec r_a) - \vec f_g(\vec r_p) - \frac{\vec F_p}{m_p}.
\end{equation}
...
\ddot{\vec{s}} = \mathbf{J}(\vec{r}_a)\,\vec{s} - \frac{\vec F_p}{m_{p}}.
\end{equation}
